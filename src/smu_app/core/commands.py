...
def process_command(cmd_str: str, app_state: AppState, ui_exiter=None):
    ...
    if not app_state.smu:
        app_state.log_message("Error: SMU not initialized.")
        return

    try:
        args = shlex.split(cmd_str)
    except ValueError as e:
        app_state.log_message(f"Syntax Error: {e}")
        return
    
    if not args: return
    
    app_state.log_message(f"> {cmd_str}")
    cmd = args[0].lower()
    ...
    try:
        if cmd in ("exit", "quit", "q"):
            ...
            app_state.log_message(HELP_TEXT)
            ...
            if len(args) < 2: app_state.log_message("Usage: vmode <voltage>")
            else:
                app_state.smu.set_voltage(float(args[1]))
                app_state.log_message(f"Set Voltage: {args[1]} V")

        elif cmd == "cmode":
            if len(args) < 2: app_state.log_message("Usage: cmode <current>")
            else:
                app_state.smu.set_current(float(args[1]))
                app_state.log_message(f"Set Current: {args[1]} A")

        elif cmd == "logdata":
            if len(args) < 2: app_state.log_message("Usage: logdata <on|off>")
            ...
            app_state.log_message(f"Limits -> V: {v_lim} V, I: {i_lim} A")
            
        else:
            app_state.log_message(f"Unknown command: '{cmd}'")

    except Exception as e:
        app_state.log_message(f"Execution Error: {e}")